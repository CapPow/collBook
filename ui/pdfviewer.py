...
class PDFViewer(QLabel):
    ...
    def load_preview(self, pdfBytes):
        try:
            ...
            mat = fitz.Matrix(self.value_Zoom, self.value_Zoom)  # the scaling / transformations
            pix = pdfPage.getPixmap(alpha = False, matrix = mat)
            fmt = QImage.Format_RGBA8888 if pix.alpha else QImage.Format_RGB888
            img = QImage(pix.samples, pix.width, pix.height, pix.stride, fmt)
            img = img.scaled(self.value_X * self.value_Zoom, self.value_Y * self.value_Zoom, Qt.KeepAspectRatio)
